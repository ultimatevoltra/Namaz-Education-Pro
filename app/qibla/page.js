...
export default function QiblaPage(){
  const{t}=useLang();const[q,setQ]=useState(null);const[hdg,setHdg]=useState(0);const[d,setD]=useState(null);const[status,setStatus]=useState('...');
  useEffect(()=>{navigator.geolocation?.getCurrentPosition(p=>{const{latitude:lat,longitude:lon}=p.coords;const qd=calcQ(lat,lon);setQ(qd);setD(dist(lat,lon));setStatus(t('qibla.title')+': '+Math.round(qd)+'° N')},()=>{setQ(calcQ(21.4225,39.8262));setStatus(t('qibla.default'))})},[t]);
  ...
  const TIPS=[t('qibla.tip1'),t('qibla.tip2'),t('qibla.tip3'),t('qibla.tip4')];
  return(<div style={{maxWidth:500,margin:'0 auto',padding:'0 16px',textAlign:'center'}}>
    <div style={{paddingTop:56,paddingBottom:16}}><h1 className="text-gold" style={{fontSize:28,fontWeight:800}}>{t('qibla.title')}</h1><p style={{color:'#6a7a8a',fontSize:14,marginTop:4}}>{t('qibla.subtitle')}</p></div>
    {d&&(<div className="card-emerald" style={{padding:'10px 20px',marginBottom:20,display:'inline-block',borderRadius:14}}><span style={{color:'#6db891',fontSize:13,fontWeight:600}}>📍 {t('qibla.dist')} </span><span style={{color:'#fff',fontSize:15,fontWeight:800}}>{d.toLocaleString()} km</span></div>)}
    ...
    <div className="card-gold" style={{padding:'14px 20px',marginBottom:16}}><div style={{fontSize:14,color:'#f0d060',fontWeight:700}}>{status}</div>{q&&<div style={{fontSize:12,color:'#8a7a4a',marginTop:4}}>{t('qibla.rotate')}</div>}</div>
    <div className="card" style={{padding:'16px',textAlign:'left'}}>
      <div style={{fontSize:12,color:'#5a6a7a',fontWeight:600,letterSpacing:0.5,marginBottom:10,textTransform:'uppercase'}}>{t('qibla.tips.title')}</div>
      {TIPS.map((tip,i)=>(<div key={i} style={{fontSize:12,color:'#8a9aaa',marginBottom:6,paddingLeft:4}}>· {tip}</div>))}
    </div>
    <div style={{height:20}}/>
  </div>);
}